<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="DoanhSoNhapKho.aspx.cs" Inherits="PRPO_Manage.Pages.BaoCao.DoanhSoNhapKho" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
     ...
     <div class="container" id="container">
         <div class="row">
            <div class="col-md-12">
                <h3>
                    <p class="text-center font-weight-bold">DOANH SỐ NHẬP KHO THEO THÁNG, THEO ĐƠN VỊ</p>
                </h3>
            </div>
        </div>
        <div class="row">
         ...
         .fail(function (jqXHR, textStatus, errorThrown) {
            ...
            for (var i = 0; i < id_pb.length; i++) {
                $.ajax({
                    type: "POST",
                    async: false,
                    url: "/Webservice/dsnguoidung.asmx/BaoCaoDoanhSoNhapKhoTheoThangTheoDonVi",
                    data: {
        ...
        function LayDataPODaDong(pb) {
            ...
            for (var s = 0; s < pb.length; s++) {
                $.ajax({
                    type: "POST",
                    async: false,
                    url: "/Webservice/dsnguoidung.asmx/BaoCaoDoanhSoNhapKhoDetail",
                    data: {
                        "id_phongban": pb[s],
                    },
                    dataType: "json",
                    success: function (data) {
                        ...
                        if (data.length > 0) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                var date = new Date(data[i]["NgayNhapKho"]);
                                ...
                                if (i % 2 == 0) {
                                    str_tr += '<tr role="row" class="odd"><td>' + data[i]["SoNhapKho"] + '</td><td>' + ngaypo + '</td><td>' + data[i]["SoLuong"] + '</td><td>' + Number(data[i]["ThanhTien"]).toLocaleString('de-DE') + '</td></tr>';
                                }
                                else {
                                    str_tr += '<tr role="row" class="even"><td>' + data[i]["SoNhapKho"] + '</td><td>' + ngaypo + '</td><td>' + data[i]["SoLuong"] + '</td><td>' + Number(data[i]["ThanhTien"]).toLocaleString('de-DE') + '</td></tr>';
                                }
